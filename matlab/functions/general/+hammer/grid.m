...
if nargin<1 || isempty(thRes_deg)
    thRes_deg = 30;
end
if nargin<2 || isempty(phRes_deg)
    phRes_deg = 60;
end
if strcmp(thRes_deg, "off")
    delete( [findobj(ax, 'tag', 'th_grid'), findobj(ax, 'tag', 'ph_grid')] );
    return
end
z = eps();
