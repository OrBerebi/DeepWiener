...
function [anm,anm_Both,anm_Direct,anm_Earl] = earo_simpwd_varN(Ns, roomDims, recPos, srcAngPos, srcType, freqAbs, alphaAbs, fs, isDiff, eqOrder)
% variable N
    ...
    if isempty(freqAbs) || isempty(alphaAbs)
        Room=SetupRoom('Dim',roomDims);
    else
        Room=SetupRoom('Dim',roomDims,'Freq',freqAbs,'Absorption',alphaAbs);
    end
