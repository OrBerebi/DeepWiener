function y=earo_bf(eobj,th_look,ph_look,beamType,beamParam,Nbf,nFFT_bf)
...
%             - modified on 23/11/14 by Jonathan Sheaffer
%             - modified on 26/11/14 by David
...
%     if (nargin<6), if (~isempty(eobj.orderN)), Nbf=eobj.orderN; else Nbf=sqrt(length(eobj.micGrid.azimuth)/2)-1; end; end;          % Default = Beamforming order is the same as microphone array order
    if (nargin<6), Nbf=sqrt(length(eobj.micGrid.azimuth)/2)-1;  end;          % Default = Beamforming order is the same as microphone array order
    if (nargin<7),nFFT_bf=min(2^15,size(eobj.data,2)); end
    ...
    useSOFiA = false;                 % change to true if you wish to generate the radial functions using SOFiA 
        ...
        otherwise
            ...
            switch beamType
                ...
                case 5,
                    ...
                    beamParam = setfield(beamParam, 'D', D3);
                    beamParam = setfield(beamParam, 'invDDH', DDH3);
                   ...
                   fprintf('\n');
                    ...
                    clear D3 DDH3 
                    beamParam = rmfield(beamParam, 'D');
                    beamParam = rmfield(beamParam, 'invDDH');
    ...
    %     figure; plot(doaTh); hold on; plot(doaPh,'r'); legend([{'\theta_{doa}'};{'\phi_{doa}'}]); ylabel('doa [rad]'); xlabel('direction index');
        ...
        Wnm=bf_design_SH(eobj.orderN,eobj.micGrid.r,doaTh,doaPh,kVec_bf,beamType,eobj.scatterer,beamParam); %[nFFT/2 x (N+1)^2 x L]
        Wnm3=permute(Wnm,[1,3,2]);


        % checks theoretical beampattern
        Ypw=shiftdim(shMatrix(eobj.orderN,th_look(1),ph_look(1)).',-1);
...
%          end
        ...
        W_delay_th=0.5;    %sets the threshold for ir delta function peak level
        W_delay_ind=find(max(abs(W_t_cs),[],2)/max(abs(W_t_cs(:)))>=W_delay_th,1,'first');
 if 0,        figure; plot(t_vec,20*log10(abs(W_t_cs(:,:,1)))); xlabel(' t [sec]'); ylabel('h_{mic} [dB]'); end
